...
var vlmenu = {
    ...
    'submitTopItemForm' : function (menu_id) {
        ...
        $.ajax({
            ...
            success : function (json) {
                if(json['submit'] === true) {
                    vlmenu.getItemsToContainer(menu_id);
                } else {
                    flag = false;
                    $('#form-container-bg').show();
                    $('#form-ajax-loader').show();
            ...
            success     : function (json) {
                ...
                if(top_items) {
                    ...
                    for(i in top_items) {
                        ...
                        html += '           <a href="javascript:void(0)" title="' + text_add_sub_item + '" onclick="vlmenu.appendAddSubItemForm(\'' + top_items[i]['menu_item_id'] + '\', \'2\')" class="a-config"><i class="fa fa-plus"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="vlmenu.appendTopItemNewForm(\'' + top_items[i]['url'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="vlmenu.deleteMenuItem(\'' + top_items[i]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                        html += '       </div>';
                        html += '   </div>';
                        ...
                        if(top_items[i]['sub_items']) {
                            ...
                            for(j in sub_items) {
                                ...
                                html += '       </div>';
                                html += '       <div class="action">';
                                html += '           <a href="javascript:void(0)" title="' + text_add_sub_item + '" onclick="vlmenu.appendAddSubItemForm(\'' + sub_items[j]['item_id'] + '\', \'3\')" class="a-config"><i class="fa fa-plus"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="vlmenu.appendEditSubItemForm(\'' + sub_items[j]['item_id'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="vlmenu.deleteMenuItem(\'' + sub_items[j]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                                html += '       </div>';
                                html += '   </div>';
                                ...
                                if(sub_items[j]['sub_items']) {
                                    ...
                                    for(k in s_items) {
                                        ...
                                        html += '       </div>';
                                        html += '       <div class="action">';
                                        html += '           <a href="javascript:void(0)" onclick="vlmenu.appendEditSubItemForm(\'' + s_items[k]['item_id'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                                        html += '           <a href="javascript:void(0)" onclick="vlmenu.deleteMenuItem(\'' + s_items[k]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                                        html += '       </div>';
                                        html += '   </div>';
            ...
            success : function (json) {
                ...
                if(child_categories) {
                    for(i in child_categories) {
                        ...
                        html += '       <a class="a-resize-minus a-left" onclick="vlmenu.decreaseColumn($(this))"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>';
                        html += '       <a class="a-resize-plus a-left" onclick="vlmenu.increaseColumn($(this))"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>';
                        html += '       <div class="widget-name-container">';
                        html += '           <span class="name">' + child_categories[i]['name'] + '</span>';
                        html += '       </div>';
                        html += '       <div class="widget-action">';
                        html += '           <a class="a-config" onclick="vlmenu.showWidgetConfiguration($(this))"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                        html += '           <a class="a-delete a-config" onclick="vlmenu.deleteWidget($(this))"><i class="fa fa-times" aria-hidden="true"></i></a>';
                        html += '       </div>';
                        html += '   </div>';
    ...
    'addWidget': function() {
        ...
        if(widget_type == 'html') {
            ...
            $.ajax({
                url : 'index.php?route=vltech/advancemenu/getLanguageData&user_token=' + user_token,
                type: 'get',
                success : function (json) {
        ...
        vlmenu.initSortMenuItem();
        vlmenu.chooseCategory();
        vlmenu.chooseProduct();
        vlmenu.initSummerNote();
        vlmenu.checkBoxClick();
    },
    ...
    'chooseCategory' : function () {
        ...
        $('.w-category').customAutoComplete({
            'source': function(request, response) {
                $.ajax({
                    url: 'index.php?route=vltech/advancemenu/autoCompleteCategory&user_token=' + user_token + '&filter_name=' +  encodeURIComponent(request),
                    dataType: 'json',
                    success: function(json) {
