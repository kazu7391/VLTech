<?php
class ControllerExtensionModuleVlmenu extends Controller
{
    public function index($setting) {
        $this->load->language('vltech/vlmenu');
        ...
        $menu = $this->model_vltech_advancemenu->getMenuById($menu_id);

        $data['menu_type'] = $menu['menu_type'];

        if($menu) {
            if($menu['status']) {
                $top_items = $this->model_vltech_advancemenu->getTopItems($menu_id);
                ...
                foreach ($top_items as $top_item) {
                    ...
                    $sub_items2 = $this->model_vltech_advancemenu->getSubItems($top_item['menu_item_id'], '2');

                    foreach ($sub_items2 as $sub_item2) {
                        $sub_items_lv3 = array();

                        $sub_items3 = $this->model_vltech_advancemenu->getSubItems($sub_item2['sub_menu_item_id'], '3');

                        foreach ($sub_items3 as $sub_item3) {
                            $third_title = $this->model_vltech_advancemenu->getSubItemDescriptionById($sub_item3['sub_menu_item_id']);
                        ...
                        $second_title = $this->model_vltech_advancemenu->getSubItemDescriptionById($sub_item2['sub_menu_item_id']);
                    ...
                    $top_item_title = $this->model_vltech_advancemenu->getTopItemDescriptionById($top_item['menu_item_id']);
        ...
        if (file_exists('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/vltech/advancemenu.css')) {
            $this->document->addStyle('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/vltech/advancemenu.css');
        } else {
            $this->document->addStyle('catalog/view/theme/default/stylesheet/vltech/advancemenu.css');
        }

        $this->document->addScript('catalog/view/javascript/vltech/advancemenu.js');

        return $this->load->view('vl/module/vlmenu', $data);
    }
